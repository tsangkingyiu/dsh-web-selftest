...
export function createWebTools(host) {
    ...
    const webNavigate = defineTool({
        ...
        async execute(args) {
            ...
            if (!session)
                throw new Error(`session ${args.sessionId} not found — call web_launch first`);
    ...
    const webScreenshot = defineTool({
        ...
        output: {
            ...
            render: renderJson,
            presentationMeta: (_args, value) => ({ kind: 'web-screenshot', path: value.path }),
        },
        async execute(args) {
            const session = host.getSession(args.sessionId);
            if (!session)
    ...
    const webInteract = defineTool({
        ...
        async execute(args) {
            ...
            let target = null;
            if (ref)
                target = session.page.locator(`aria-ref=${ref}`);
            else if (selector)
                target = session.page.locator(selector);
            ...
            if (action === 'click' || action === 'press')
                await viz.move(vizTarget, session.page);
            if (action === 'type' && selector)
                await viz.typing(selector, session.page);
            switch (action) {
                case 'click':
                    if (target)
                        await target.click();
                        ...
                        throw new Error('click requires ref, selector, or coordinates');
            ...
            if (action === 'type') {
                ...
                if (selector)
                    await makeVisualizer().typing(selector, session.page).catch(() => { });
            ...
            let matched = true;
            if (expect_text)
                matched = (await session.page.textContent('body'))?.includes(expect_text) ?? false;
            if (expect_gone)
                matched = !((await session.page.textContent('body'))?.includes(expect_gone) ?? false);
            if (expect_url)
                matched = session.page.url().includes(expect_url);
            const dir = join(tmpdir(), 'dsh-web-selftest', args.sessionId);
            mkdirSync(dir, { recursive: true });
