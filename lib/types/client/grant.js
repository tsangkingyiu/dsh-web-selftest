 ...
 */
export async function grantScreenshotUrl(path) {
...
export async function grantStreamUrl(sessionId) {
...
export async function fetchConsoleEntries(sessionId) {
    ...
    if (!res.ok)
        throw new Error(`console fetch failed (${res.status})`);
    const body = await res.json();
    if (!isRecord(body) || !Array.isArray(body.entries))
        throw new Error('malformed console answer');
