...
export function screenshotDir(sessionId) {
    return join(tmpdir(), 'dsh-web-selftest', sessionId);
}
async function readKeyFile(path) {
    const info = await lstat(path);
    if (info.isSymbolicLink() || !info.isFile())
        throw new Error('dsh-web-selftest stream access key is not a regular file');
    const key = await readFile(path);
    if (key.length !== KEY_BYTES)
        throw new Error('dsh-web-selftest stream access key has an invalid length');
    return key;
}
...
export class StreamAccessController {
    ...
    async signStreamToken(sessionId) {
        return this.#sign({ v: 1, kind: 'web-stream', sessionId });
    }
    async signScreenshotToken(path) {
        if (!isAbsolute(path))
            throw new TypeError('dsh-web-selftest: signScreenshotToken requires an absolute path');
        return this.#sign({ v: 1, kind: 'web-screenshot', path });
    }
    async verifyStreamToken(token) {
        const payload = await this.#verify(token);
        if (payload?.kind !== 'web-stream' || typeof payload.sessionId !== 'string')
            return undefined;
        return { sessionId: payload.sessionId };
    }
    async verifyScreenshotToken(token) {
        const payload = await this.#verify(token);
        if (payload?.kind !== 'web-screenshot' || typeof payload.path !== 'string')
            return undefined;
...
function isTrustedRequest(req, requireOrigin) {
    ...
    if (authority === undefined || !TRUSTED_AUTHORITIES.has(authority.hostname))
        return false;
    ...
    ctx.inject(['webServer'], (webCtx) => {
        ...
        disposers.push(webCtx.webServer.register({
            ...
            handler: async (req, res) => {
                ...
                res.writeHead(200, { 'content-type': 'application/json' });
                const entries = [
                    ...session.consoleMessages.map(m => ({ kind: 'console', type: m.type, text: m.text, timestamp: m.timestamp })),
                    ...session.pageErrors.map(e => ({ kind: 'pageerror', text: e.message, timestamp: e.timestamp })),
