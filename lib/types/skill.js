...
export const WEB_SKILL_DESCRIPTION = 'Drive a headless Chromium browser: read the page, click, type, scroll, and confirm that an action landed. Read this before the first web_* call of a UI task.';
export const WEB_SKILL_WHEN_TO_USE = 'Any task that operates a web page through the web_* tools — opening pages, clicking controls, filling fields, scrolling, or verifying what is on screen.';
...
`;
export function registerWebSkill(ctx) {
    const fiber = ctx.inject(['skills'], (skillCtx) => {
        skillCtx.skills.register({
            ...
            // Required by SkillRegistration (Omit<SkillDefinition,'invocation'|'provider'>):
            // validated only at skills.get() load time, not at register() — its absence
            // surfaced as "loaded skill ... source must be a string" on first use.
            source: 'custom',
